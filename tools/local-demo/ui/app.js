...
function buildMapGeoJson(trace, providers) {
  ...
  for (const segment of routeSegments(trace)) {
    ...
    features.push({
      ...
      properties: { kind: "route", good: isGood(segment.result), label: `${from.label} to ${to.label}: ${segment.result}` },
    });
  }
...
function addMapLayers() {
  ...
  state.map.addLayer({ id: "nodes", type: "circle", source: "flareless-live", filter: ["==", ["get", "kind"], "node"], paint: { "circle-radius": ["case", ["==", ["get", "id"], "flareless"], 10, 7], "circle-color": ["get", "color"], "circle-stroke-color": "#e8f5ff", "circle-stroke-width": 1.2, "circle-blur": 0.02 } });
  ...
  state.map.moveLayer("node-glow", "nodes");
...
function animateRouteArcs() {
  setInterval(() => {
    ...
    const lead = 0.1 + state.animationTick * 0.08;
    state.map.setPaintProperty("routes-flow", "line-dasharray", [lead, 2.2, 0.1, 1.6]);
  }, 140);
}
...
function providerStatusClass(provider) {
  if ((provider.lastResult || "").match(/TIMEOUT|BLOCKED|HTTP_|ERROR|OFFLINE/)) return "failed";
  if (provider.status && provider.status !== "healthy") return "degraded";
  return "healthy";
}
function providerBadge(provider) {
  const cls = providerStatusClass(provider);
  if (cls === "failed") return '<span class="badge fail">failed</span>';
  if (cls === "degraded") return '<span class="badge warn">degraded</span>';
...
function updateProviderMarkers(providers) {
  ...
  for (const provider of providers) {
    ...
    const html = `<div class="provider-marker ${statusClass}"><b>${providerDisplayName(provider)}</b><div class="marker-row"><span class="marker-dot"></span>${provider.latencyMs || "--"} ms · ${provider.lastResult || "standby"}</div></div>`;
    if (!state.providerMarkers[provider.name]) {
      const element = document.createElement("div");
      element.innerHTML = html;
      const marker = new maplibregl.Marker({ element: element.firstElementChild, anchor: "left", offset: [12, -12] }).setLngLat([node.lon, node.lat]).addTo(state.map);
      state.providerMarkers[provider.name] = marker;
    } else {
      const markerElement = state.providerMarkers[provider.name].getElement();
      markerElement.className = `provider-marker ${statusClass}`;
      markerElement.innerHTML = `<b>${providerDisplayName(provider)}</b><div class="marker-row"><span class="marker-dot"></span>${provider.latencyMs || "--"} ms · ${provider.lastResult || "standby"}</div>`;
      state.providerMarkers[provider.name].setLngLat([node.lon, node.lat]);
    }
    state.providerMarkers[provider.name].getElement().style.borderColor = provider.name === activeProvider ? "rgba(70,240,160,.85)" : "rgba(255,255,255,.14)";
  }
}
...
async function refresh() {
  ...
  if (state.mapReady) {
    state.map.getSource("flareless-live")?.setData(buildMapGeoJson(state.routeTrace, state.providers));
    updateProviderMarkers(state.providers);
  }
  renderAll(status);
}
...
function updateHeader(status) {
  ...
  if (!reason || reason === "UNKNOWN") {
    globalStatus.textContent = "STATUS: PAUSED";
  } else if (reason.includes("SUCCESS") || reason.includes("ADVERTISES_CONTENT")) {
    globalStatus.textContent = "STATUS: OPTIMAL";
  } else if (reason.includes("FAILED") || reason.includes("BLOCKED") || reason.includes("NO_HEALTHY")) {
...
function renderProviders() {
  ...
  table.innerHTML = state.providers.map((provider) => `<tr><td>${providerDisplayName(provider)}</td><td>${providerBadge(provider)}</td><td>${provider.latencyMs || "--"} ms</td><td>${provider.lastResult || "--"}</td><td>${provider.name === state.activeProvider ? "active" : "standby"}</td></tr>`).join("");
}

function renderApprovals() {
  const container = $("approvalCards");
  if (!state.recommendations.length) {
    container.textContent = "No recommendations loaded.";
    return;
  }
  container.innerHTML = state.recommendations.map((item) => `<div class="approval-card"><b>${item.summary}</b><div class="approval-meta">${item.recommendationId} · ${item.status} · ${item.severity} · ${item.createdAt || ""}</div><div>${(item.reasonCodes || []).join(", ") || "No reason codes"}</div></div>`).join("");
}

function renderPeers() {
  const rows = [
    ["hashed3365627006...", "Online", "250MB", "Hash 100%", "ok"],
    ["hashed5964045005...", "Online", "250MB", "Hash 100%", "ok"],
    ["hashed3320052003...", "Online", "500MB", "Hash 100%", "ok"],
  ];
...
function renderPolicyCode() {
  const policy = {
    policy: "video-public-peer-assisted-failover",
    when: state.conditions,
    then: ["route_to_next_healthy_cdn", "route_to_hash_verified_micro_cdn"],
    safeguards: { operatorApproval: true, hashVerification: true, livePolicyMutation: false, saveEnabled: false },
  };
  if (state.policyFormat === "json") {
    $("policyCode").textContent = JSON.stringify(policy, null, 2);
    return;
  }
  $("policyCode").textContent = `policy: ${policy.policy}\nwhen:\n${state.conditions.map((item) => `  - ${item}`).join("\n")}\nthen:\n  - route_to_next_healthy_cdn\n  - route_to_hash_verified_micro_cdn\nsafeguards:\n  operatorApproval: true\n  hashVerification: true\n  livePolicyMutation: false\n  saveEnabled: false`;
}

function renderConditions() {
  $("conditionList").innerHTML = state.conditions.map((item, index) => `<div class="rule-line"><button>Condition ${index + 1}</button><span>${item}</span></div>`).join("");
  renderPolicyCode();
}
...
async function decideLatest(action) {
  const item = latestPendingRecommendation();
  if (!item) return toast("No pending recommendation.");
  await api(`/agent/recommendations/${item.recommendationId}/${action}`, { operator: "local-operator", note: `${action} from UI phase one` });
  await refresh();
  toast(`Recommendation ${action}d.`);
}
async function runScenario() { await api("/route/simulate", { scenarioId: $("scenarioSelect").value }); await refresh(); toast("Scenario applied."); }
async function applyOutage() { $("scenarioSelect").value = "http-status-failover"; await runScenario(); }
function startPolling() { if (state.timer) return; state.timer = setInterval(refresh, 1000); $("pollBtn").disabled = true; $("pauseBtn").disabled = false; refresh(); }
function pausePolling() { if (state.timer) clearInterval(state.timer); state.timer = null; $("pollBtn").disabled = false; $("pauseBtn").disabled = true; updateHeader({ routeReason: $("routeReason").textContent }); }
function showView(viewId) { state.currentPage = viewId; document.querySelectorAll(".nav-item").forEach((button) => button.classList.toggle("active", button.dataset.view === viewId)); document.querySelectorAll(".view").forEach((view) => view.classList.toggle("active", view.id === viewId)); if (viewId === "dashboard") setTimeout(() => state.map?.resize(), 50); }
...
function bindEvents() {
  ...
  $("approveBtn").addEventListener("click", () => decideLatest("approve"));
  $("rejectBtn").addEventListener("click", () => decideLatest("reject"));
