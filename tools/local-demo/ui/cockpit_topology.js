...
// This file owns only SVG topology and cockpit metrics. It never creates or mutates the Dashboard MapLibre map.

let topologyDrag = null;
const TOPOLOGY_BASE_W = 900;
const TOPOLOGY_BASE_H = 420;
...
function topologySvgSize() {
  const svg = $("topologySvg");
  const rect = svg?.getBoundingClientRect?.();
  return {
    width: Math.max(420, Math.round(rect?.width || TOPOLOGY_BASE_W)),
    height: Math.max(280, Math.round(rect?.height || TOPOLOGY_BASE_H)),
  };
}

function dynamicTopologyRadius(nodeCount, configuredRadius, scale) {
  const base = Number(configuredRadius || 38);
  const crowded = nodeCount <= 7 ? base : Math.min(base, Math.max(18, 44 - Math.ceil((nodeCount - 7) * 2.4)));
  return Math.max(12, Math.round(crowded * scale));
}

function layoutTopologyNodes(size = topologySvgSize()) {
  const raw = topologyNodes().map((node) => ({ ...node }));
  const count = raw.length;
  const scaleX = size.width / TOPOLOGY_BASE_W;
  const scaleY = size.height / TOPOLOGY_BASE_H;
  const scale = Math.min(scaleX, scaleY);
  const margin = 42;
  return raw.map((node) => {
    const baseX = Math.max(margin, Math.min(TOPOLOGY_BASE_W - margin, Number(node.x ?? 120)));
    const baseY = Math.max(70, Math.min(TOPOLOGY_BASE_H - margin, Number(node.y ?? 120)));
    return {
      ...node,
      baseX,
      baseY,
      ...
      r: dynamicTopologyRadius(count, node.r || 38, scale),
      scaleX,
      scaleY,
    };
  });
...
function renderTopology() {
  const svg = $("topologySvg");
  if (!svg) return;
  const size = topologySvgSize();
  svg.setAttribute("viewBox", `0 0 ${size.width} ${size.height}`);
  svg.setAttribute("preserveAspectRatio", "none");
  const tNodes = layoutTopologyNodes(size);
  ...
  const legendY = Math.max(88, size.height - 70);
...
function attachTopologyDragHandlers(size = topologySvgSize()) {
  const svg = $("topologySvg");
  if (!svg) return;
  const scaleX = size.width / TOPOLOGY_BASE_W;
  const scaleY = size.height / TOPOLOGY_BASE_H;
...
window.addEventListener("mousemove", (event) => {
  ...
  if (!svg) return;
  const p = svgPoint(svg, event);
  const dx = (p.x - topologyDrag.startX) / Math.max(0.001, topologyDrag.scaleX || 1);
  const dy = (p.y - topologyDrag.startY) / Math.max(0.001, topologyDrag.scaleY || 1);
  const node = state.topologyConfig.nodes.find((item) => item.id === topologyDrag.id);
  if (!node) return;
  node.x = Math.round(Math.max(38, Math.min(TOPOLOGY_BASE_W - 38, topologyDrag.nodeX + dx)));
  node.y = Math.round(Math.max(64, Math.min(TOPOLOGY_BASE_H - 38, topologyDrag.nodeY + dy)));
...
function cockpitValue(value, max) { const pct = Math.max(0, Math.min(100, Math.round((Number(value || 0) / Math.max(1, max)) * 100))); return `${pct}%`; }
function metricAverage(values) { return Math.round((values || []).reduce((a, b) => a + Number(b || 0), 0) / Math.max(1, (values || []).length)); }
function defaultMetricWidgets() { return [{ id: "route-attitude", type: "route-attitude", title: "Route Attitude", locked: false }, { id: "primary-instruments", type: "primary-instruments", title: "Primary Instruments", locked: false }, { id: "provider-matrix", type: "provider-matrix", title: "Provider Matrix", locked: false }, { id: "event-tape", type: "event-tape", title: "Event Tape", locked: false }, { id: "traffic-distribution", type: "traffic-distribution", title: "Traffic Distribution", locked: false }, { id: "operator-notes", type: "custom", title: "User Widget Bay", body: "Write your own local widget notes here. Add more custom widgets from the Metrics header.", locked: false }]; }
function loadMetricWidgets() { if (Array.isArray(state.metricsWidgets)) return; try { const saved = JSON.parse(localStorage.getItem("flareless.metrics.widgets") || "[]"); state.metricsWidgets = Array.isArray(saved) && saved.length ? saved : defaultMetricWidgets(); } catch { state.metricsWidgets = defaultMetricWidgets(); } }
function saveMetricWidgets() { localStorage.setItem("flareless.metrics.widgets", JSON.stringify(state.metricsWidgets || [])); }
function ensureMetricsWidgetControls() { const head = document.querySelector("#metrics .card-head"); if (!head || document.getElementById("metricWidgetType")) return; const controls = document.createElement("div"); controls.className = "metrics-widget-controls"; controls.innerHTML = `<select id="metricWidgetType"><option value="custom">Custom written widget</option><option value="primary-instruments">Primary Instruments</option><option value="provider-matrix">Provider Matrix</option><option value="event-tape">Event Tape</option><option value="traffic-distribution">Traffic Distribution</option><option value="route-attitude">Route Attitude</option></select><input id="metricWidgetTitle" placeholder="widget title"/><input id="metricWidgetBody" class="custom-body" placeholder="custom widget text"/><button id="addMetricWidgetBtn">Add Widget</button><button id="resetMetricWidgetsBtn">Reset</button>`; head.appendChild(controls); document.getElementById("addMetricWidgetBtn").addEventListener("click", addMetricWidgetFromControls); document.getElementById("resetMetricWidgetsBtn").addEventListener("click", () => { state.metricsWidgets = defaultMetricWidgets(); saveMetricWidgets(); renderMetrics(); toast("Metrics widgets reset."); }); }
function addMetricWidgetFromControls() { loadMetricWidgets(); const type = document.getElementById("metricWidgetType")?.value || "custom"; const titleInput = document.getElementById("metricWidgetTitle"); const bodyInput = document.getElementById("metricWidgetBody"); const title = titleInput?.value.trim() || ({ custom: "Custom Widget", "primary-instruments": "Primary Instruments", "provider-matrix": "Provider Matrix", "event-tape": "Event Tape", "traffic-distribution": "Traffic Distribution", "route-attitude": "Route Attitude" }[type] || "Metric Widget"); const body = bodyInput?.value.trim() || "Custom written widget content. Use this for operator notes, runbook links, or site specific counters."; state.metricsWidgets.push({ id: `${type}-${Date.now()}`, type, title, body, locked: false }); if (titleInput) titleInput.value = ""; if (bodyInput) bodyInput.value = ""; saveMetricWidgets(); renderMetrics(); recordEvent("METRIC_WIDGET_ADDED", { type, title }); toast("Metric widget added."); }
function removeMetricWidget(id) { loadMetricWidgets(); state.metricsWidgets = state.metricsWidgets.filter((widget) => widget.id !== id); saveMetricWidgets(); renderMetrics(); recordEvent("METRIC_WIDGET_REMOVED", { id }); toast("Metric widget removed."); }
function cockpitPanel(id, title, subtitle, body, extraClass = "") { return `<div class="cockpit-panel ${extraClass}" data-widget-id="${escapeHtml(id)}"><div class="cockpit-panel-head"><div><b>${escapeHtml(title)}</b>${subtitle ? `<small>${escapeHtml(subtitle)}</small>` : ""}</div><button class="widget-remove" onclick="removeMetricWidget('${escapeHtml(id)}')">×</button></div>${body}</div>`; }
...
window.addEventListener("resize", () => {
  if (state.currentPage === "topology") setTimeout(renderTopology, 80);
});

injectCockpitTopologyStyles();
renderMetrics();
renderTopology();
recordEvent("COCKPIT_TOPOLOGY_READY", { draggable: true, cockpit: true, userWidgets: true, topologyAutoFill: "isolated-svg" });
